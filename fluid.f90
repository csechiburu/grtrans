	  ...
	  use fluid_model_blob, only: blob_vals, init_blob, del_blob
	  use fluid_model_sariafblob, only: init_sariafblob, del_sariafblob
      ...
      integer, parameter :: SARIAF=17,POWERLAW=18,HARM3D=19,HARMPI=20,TOY=21,KORAL=22,KORALNTH=23, &
           SHELL=24,KORAL3D=25,KORAL3D_DISK=26,KORAL3D_TOPJET=27,KORAL3D_BOTJET=28,IHARM=29,BLOB=30, &
		   SARIAFBLOB=31
...
!      real :: sigcut=1.e10
      ...
      type fluid_args
         ...
         real(8) :: rspot,r0spot,n0spot,tscl,rscl,wmin,wmax,fmin, &
              ...
              r_c,theta_c,phi_c,n_e,t_e,b_b,r_b
      end type
      ...
      contains
        ...
        subroutine assign_fluid_args(fargs,dfile,hfile,gfile,sim,nt,indf,nfiles,jonfix, &
          ...
          real(8), intent(in) :: rspot,r0spot,n0spot,tscl,rscl,wmin,wmax,fmin, &
               ...
               rin,rout,thin,thout,phiin,phiout,scalefac,sigcut,r_c,theta_c,phi_c,n_e,t_e,b_b,r_b
		  ...
		  fargs%r_c = r_c; fargs%theta_c=theta_c; fargs%phi_c = phi_c; fargs%n_e = n_e
		  fargs%t_e = t_e; fargs%b_b = b_b; fargs%r_b = r_b
...
!           write(*,*) 'after load fluid model, sariaf nscl is', real(fargs%nscl)
!           call init_sariaf()
    ...
    !      write(6,*) 'load'
        ...
        elseif(fname=='SCHNITTMAN') then
           call init_schnittman_hotspot(ifile,real(fargs%rspot),real(fargs%r0spot) &
                ,real(fargs%n0spot))
        elseif(fname=='BLOB') then
			!call init_blob()
           call init_blob(real(fargs%r_c),real(fargs%theta_c),real(fargs%phi_c), &
			real(fargs%n_e),real(fargs%t_e),real(fargs%b_b),real(fargs%r_b))
	    elseif(fname=='SARIAFBLOB') then
	       call init_sariafblob(real(fargs%nscl),real(fargs%tscl),real(fargs%nnthscl), &
                real(fargs%nnthp),real(fargs%beta),fargs%bl06,real(fargs%r_c),real(fargs%theta_c), &
				real(fargs%phi_c),real(fargs%n_e),real(fargs%t_e),real(fargs%b_b),real(fargs%r_b))
        endif
 ...
 ! !      write(6,*) 'fluid: ',size(x0),f%model
      ...
      !      write(6,*) 'made it'
          ...
          CASE (BLOB)
             call get_blob_fluidvars(x0,f)
          CASE (SARIAFBLOB)
             call get_sariafblob_fluidvars(x0,real(a),f)
          CASE (DUMMY)
        END SELECT
        ...
        SELECT CASE(f%model)
          ...
          CASE (BLOB)
             call convert_fluidvars_blob(f,ncgs,ncgsnth,bcgs,tcgs,sp)
          CASE (SARIAFBLOB)
             call convert_fluidvars_sariafblob(f,ncgs,ncgsnth,bcgs,tcgs,sp)
        END SELECT
...
! debugging stuff
        ...
        psi4 = 2.*rr/rho2
        !write(*,*) 'sariaf pars before calling sariaf_vals', a,ctheta,u,n,t,bmag,riaf_vr,riaf_vth,riaf_omega,nnth,bl06
...
!        not necessary to scale f%b because f%b is only used for an angle
	...
	!ADDED BY CONI
	subroutine get_blob_fluidvars(x0,f)
	type(four_vector), intent(in), dimension(:) :: x0
	type(fluid), intent(inout) :: f
	! input into blob_vals
	real :: n_e, t_e, b_b
	! outputs from blob_vals
	real, dimension(size(x0)) :: n_arr, T_arr, B_arr, weights ! these are the arrays with constan n_e, T_e and B values
	! Other variables needed: g00=1st component of Schwz, grr=2nd, gtt=3rd (t of theta not time)
	real, dimension(size(x0)) :: u,g00,grr,gtt
	! Assign position
	u = 1.0/(x0%data(2))  ! this is 1/r, not velocity
	call blob_vals(real(x0%data(2)),real(x0%data(3)),real(x0%data(4)),n_e,t_e,b_b,n_arr,T_arr,B_arr,weights) ! calling blob_vals for r, theta, phi. Passing blob properties and returning them as arrays 
	g00 = -(1.0-2.0*u)
	grr = -1.0/g00
	!gtt = 1.0/u**2 !(t of theta not time)
	! construct 4-velocity
	f%u%data(1)=sqrt(-1.0/g00)
	f%u%data(2)=0.
	f%u%data(3)=0.
	f%u%data(4)=0.
	! B components from u*B=0 and Bmag = sqrt(B^mu*B_mu)
	f%b%data(1)=0.
	f%b%data(2)= B_arr/sqrt(grr)
	f%b%data(3)= 0.!B_arr/sqrt(gtt)
	f%b%data(4)=0.

	f%bmag = B_arr
	f%rho = n_arr
	f%p = T_arr
	end subroutine get_blob_fluidvars
	
	subroutine get_sariafblob_fluidvars(x0,a,f) 
	type(four_vector), intent(in), dimension(:) :: x0
	type(fluid), intent(inout) :: f
	! ######### variables used for blob stuff ######### 
	real, dimension(size(x0)) :: u,g00,grr,ur
	real :: n_e, t_e, b_b
	real, dimension(size(x0)) :: n_arr, T_arr, B_arr, weights
	! ######### variables used for sariaf stuff ######### 
    real,intent(in) :: a
    real, dimension(size(x0)) :: ctheta
	real, dimension(size(x0)) :: riaf_vr,riaf_vth,riaf_omega, &
             bmag,n,t,nnth
    real, dimension(size(x0)) :: ub,aleph,bb
    real, dimension(size(x0)) :: rr, rho2,psi4,stheta
    ...
    real, dimension(size(x0)) :: delta,hhh 
    real(8), dimension(size(x0),10) :: metric
    real(8), dimension(size(x0)) :: hc,lc,d,ar,om,u0
    integer :: bl06
	type (four_vector), dimension(size(x0)) :: fu
	! ######### end of sariaf stuff ######### 
	u = 1.0/(x0%data(2))  ! this is 1/r, not velocity
	! more sariaf stuff before calling sariaf_vals
    rr = x0%data(2)
    rms = calc_rms(a)
    lambdae = (rms**2. - 2.*a*sqrt(rms) + a**2.)/(rms**(3./2.)-2.*sqrt(rms)+a)
    delta = rr*rr - 2.*rr + a*a
	...
	call blob_vals(real(x0%data(2)),real(x0%data(3)),real(x0%data(4)),n_e,t_e,b_b,n_arr,T_arr,B_arr,weights)
	!write(*,*) 'B_arr in sariafblob fluidvars: ', b_arr
	!write(*,*) 'weights: ', weights
	where(weights.eq.1d0)
		! from get_blob_fluidvars
		g00 = -(1.0-2.0*u)
		grr = -1.0/g00
		! construct 4-velocity
		f%u%data(1)=sqrt(-1.0/g00)
	...
	!f%rho = sariafblob_te
	!f%p = sariafblob_b
    end subroutine get_sariafblob_fluidvars
		
        subroutine convert_fluidvars_blob(f,ncgs,ncgsnth,bcgs,tcgs,sp)
          type (fluid), intent(in) :: f
          type (source_params), intent(in) :: sp
          real(kind=8), dimension(size(f%rho)),  &
          intent(inout) :: ncgs,ncgsnth,bcgs,tcgs
          ...
          !write(6,*) 'convert',size(f%rho),size(f%bmag),size(f%p)
!		  write(*,*) 'frho', f%rho
          ncgs=f%rho; bcgs=f%bmag; tcgs=f%p; ncgsnth=f%rho
!          write(*,*) 'Pars after convert: ', ncgs!, bcgs, tcgs, ncgsnth
        end subroutine convert_fluidvars_blob

        subroutine convert_fluidvars_sariafblob(f,ncgs,ncgsnth,bcgs,tcgs,sp)
          type (fluid), intent(in) :: f
          type (source_params), intent(in) :: sp
          real(kind=8), dimension(size(f%rho)),  &
          intent(inout) :: ncgs,ncgsnth,bcgs,tcgs
          ncgs=f%rho; bcgs=f%bmag; tcgs=f%p; ncgsnth=f%rho
        end subroutine convert_fluidvars_sariafblob
